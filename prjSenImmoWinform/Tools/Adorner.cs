...
namespace prjSenImmoWinform.Tools
{
    static class Adorner
    {
        ...
        static public bool AddBadgeTo(Control ctl, string Text, Color backColor, Color forecolor)
        {
            if (controls.Contains(ctl)) return false;

            Badge badge = new Badge(backColor, forecolor);
            badge.AutoSize = true;
            badge.Text = Text;
        ...
        class Badge : Label
        {
            ...
            public Badge(Color bckColor, Color foreColor)
            {
                BackColor = bckColor;
                ForeColor = foreColor;
